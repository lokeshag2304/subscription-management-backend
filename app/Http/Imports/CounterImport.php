 ...
 * CounterImport
 *
 * Excel columns: domain | client_id | product_id | vendor_id | amount | renewal_date | grace_period | remarks
 * Table: counters
 */
class CounterImport extends SmartImporter implements \Maatwebsite\Excel\Concerns\WithHeadingRow
{
    protected function tableName(): string   { return 'counters'; }
    protected function clientTable(): string { return 'superadmins'; }

    protected function columnHeaders(): array
    {
        return [
            'domain', 'client_id', 'product_id', 'vendor_id', 'amount', 
    ...
    protected function existingDupKeys(array $parsedRows): array
    {
        ...
        DB::table('counters')->select(['domain_master_id', 'product_id', 'vendor_id', 'renewal_date', 'client_id'])
            ->whereIn('domain_master_id', $dmIds)
            ->whereIn('product_id', $productIds)
            ->whereIn('vendor_id', $vendorIds)
            ->whereIn('client_id', $clientIds)
            ->whereIn('renewal_date', $dates)
            ->get()->each(function ($s) use (&$set) {
    ...
    protected function parseRow(\Illuminate\Support\Collection $row, int $rowNumber): ?array
    {
        ...
        // 1. deletion_date = renewal_date + 1 day
        $deletionDate = \Illuminate\Support\Carbon::parse($renewalDate)->addDay()->format('Y-m-d');
        
        // 2. due_date calculation from GracePeriodService
