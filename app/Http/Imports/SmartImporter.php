...
abstract class SmartImporter implements ToCollection, WithChunkReading
{
    ...
    protected array $domainMasterCache = [];
    ...
    protected array $domainMasterNamesMap = [];
     ...
     * @return array|null
     */
    abstract protected function parseRow(\Illuminate\Support\Collection $row, int $rowNumber): ?array;
     ...
     * @return array
     */
    abstract protected function rawRowToArray(\Illuminate\Support\Collection $row): array;
    ...
    public final function collection(\Illuminate\Support\Collection $rows)
    {
        Log::info('[Import] collection() called — total rows in chunk: ' . $rows->count());
        $this->warmCaches();
        ...
        foreach ($rows as $index => $row) {
            // Check for header row (only if collection uses numeric indexes)
            if ($this->totalRows === 0 && $index === 0 && isset($row[0])) {
                $h = $this->columnHeaders()[0] ?? 'Product';
                if (strtolower($this->clean($row[0] ?? '')) === strtolower($this->clean($h))) {
                    Log::info('[Import] Row 0 detected as header row — skipping.');
                    continue;
                }
            }
            ...
            if ($isEmpty) {
                Log::info('[Import] Row ' . $index . ' is empty — skipping.');
                continue;
            }
            ...
            Log::info('[Import] Processing row ' . $rowNumber . ': ' . json_encode($row->toArray()));
            ...
            try {
                ...
                Log::error('[Import] parseRow() threw exception on row ' . $rowNumber . ': ' . $e->getMessage());
                continue;
            }

            if ($parsed === null) {
                $this->failed++;
                Log::warning('[Import] Row ' . $rowNumber . ' returned null from parseRow() — counted as failed. Errors so far: ' . json_encode($this->errors));
                continue;
            }

            if ($parsed === []) {
                Log::warning('[Import] Row ' . $rowNumber . ' returned [] from parseRow() — critical fields missing, skipped silently. Raw: ' . json_encode($row->toArray()));
                continue;
            }

            Log::info('[Import] Row ' . $rowNumber . ' parsed OK: ' . json_encode($parsed));
        ...
        Log::info('[Import] Rows ready for insert: ' . count($parsedRowsData) . ' | Failed: ' . $this->failed . ' | Errors: ' . json_encode($this->errors));

        if (empty($parsedRowsData)) {
            Log::warning('[Import] parsedRowsData is empty — nothing to insert. Returning early.');
            return;
        }
        ...
        foreach ($parsedRowsData as $i => $parsed) {
            ...
            if (isset($existingSet[$dupKey])) {
                ...
                Log::info('[Import] Row ' . ($i + 1) . ' is a DUPLICATE — key: ' . $dupKey);
                continue;
            }

            $existingSet[$dupKey] = true;
            $inserts[] = $this->buildInsertRow($parsed, $nowTimestamp);
            Log::info('[Import] Row ' . ($i + 1) . ' queued for insert — key: ' . $dupKey);
        }

        Log::info('[Import] Total queued for DB insert: ' . count($inserts) . ' | Duplicates so far: ' . $this->duplicates);
        ...
        if (!empty($inserts)) {
            try {
                ...
                Log::info('[Import] Batch insert SUCCESS — ' . count($inserts) . ' records inserted into ' . $this->tableName());
            } catch (\Throwable $e) {
                Log::error('[Import] Batch insert FAILED — falling back to row-by-row. Error: ' . $e->getMessage());
                // Fallback to row-by-row
                foreach ($inserts as $idx => $insertData) {
                    try {
                        DB::table($this->tableName())->insert($insertData);
                        $this->inserted++;
                        Log::info('[Import] Row-by-row insert SUCCESS for row ' . ($rowNumbers[$idx] ?? $idx));
                    } catch (\Throwable $ex) {
                        if (isset($ex->errorInfo[1]) && $ex->errorInfo[1] === 1062) {
                            $this->duplicates++;
                            $this->duplicateRows[] = $this->rawRowToArray(collect($rawRowsData[$idx]));
                            Log::warning('[Import] Row-by-row: MySQL duplicate (1062) for row ' . ($rowNumbers[$idx] ?? $idx));
                        } else {
                            $this->failed++;
                            $this->errors[] = ['row' => $rowNumbers[$idx], 'reason' => $ex->getMessage()];
                            Log::error('[Import] Row-by-row insert FAILED for row ' . ($rowNumbers[$idx] ?? $idx) . ': ' . $ex->getMessage() . ' | Data: ' . json_encode($insertData));
                        }
                    }
                }
            }
    ...
    protected function warmCaches(): void
    {
        ...
        // Domain Master cache — used by Subscription
        DB::table('domain_master')->get(['id', 'domain_name'])->each(function ($dm) {
            $key = $this->normalize($dm->domain_name);
            if (!isset($this->domainMasterCache[$key])) $this->domainMasterCache[$key] = (int)$dm->id;
            $this->domainMasterCache[(string)$dm->id] = (int)$dm->id;
            $this->domainMasterNamesMap[(int)$dm->id] = $key;
        });
    }
    ...
    protected function smartResolve(string $table, string $name, array &$cache, bool $allowCreate = true): int
    {
        ...
        if ($table === 'domain_master') {
            $data = ['domain_name' => $name, 'created_at' => now()];
        } else {
