...
public function login(Request $request)
{
    ...
    $roleName = [1 => 'Superadmin', 2 => 'User', 3 => 'Client'][$matchedAdmin->login_type] ?? 'User';
    ...
    try {
        \App\Services\ActivityLogger::logActivity(
            ...
            null, 
            null, 
            "$decryptedName ($roleName) Logged In at $currentTime",
            $request
        );
    } catch (\Exception $e) {}
    ...
    $whatsapp_status = $setting->whatsapp_status ?? 0;
    $email_status = $setting->email_status ?? 0;
    $sms_status = $setting->sms_status ?? 0;
    ...
    return response()->json([
        ...
        'logo' => $logo,
        'favicon' => $favicon,
        'whatsapp_status' => $isOtpRequired ? $whatsapp_status : 0,
        'email_status' => $isOtpRequired ? $email_status : 0,
        'sms_status' => $isOtpRequired ? $sms_status : 0,
...
public function two_step_otp(Request $request)
{
    ...
    if (!empty($otp_type)) {
        ...
        if ($otp_type == 'email' && !empty($superadmin->email)) {
            ...
            $message = "We have sent an OTP to your registered email {$first_three}******* successfully. The OTP will be valid for 10 minutes only.";
...
Your OTP for the Subscription portal is {1}. Please do not share it with anyone.';
    ...
    // Fetch updated user and remove sensitive OTP info before returning
    $updatedUser = DB::table('superadmins')->where('id', $data['id'])->first();
    if ($updatedUser) {
        unset($updatedUser->otp);
        unset($updatedUser->otp_expiry);
    }
   ...
   public function verifyOtpForForget(Request $request)
{
    ...
    if (!empty($user)) {
        ...
        $changepasswordurl = env('FRONTEND_URL', 'https://fsisubscriptions.testingscrew.com') . "/auth/change-password/" . $forget_string;
...
public function verifyOtp(Request $request)
{
    ...
    // Activity log for all roles
    $currentTime = Carbon::now()->setTimezone('Asia/Kolkata')->format('Y-m-d h:i A');
    $roleName = [1 => 'Superadmin', 2 => 'User', 3 => 'Client'][$user->login_type] ?? 'User';
    ...
    try {
        \App\Services\ActivityLogger::logActivity(
            ...
            null, 
            null, 
            "{$user->name} ($roleName) Logged In via OTP at $currentTime",
            $request
        );
    } catch (\Exception $e) {}
    ...
    {
        return 
            "<p>Hey ".$mailData['customer_name'].",</p>"
            ."<p>We received a request to reset your password. Click the button below to reset it:</p>"
            ."<p><a href='".$mailData['reset_link']."' style='display:inline-block;padding:10px 20px;background-color:#4f46e5;color:#ffffff;text-decoration:none;border-radius:6px;font-weight:bold;'>Reset Now</a></p>"
            ."<p>If you didn't request a password reset, you can safely ignore this email.</p>"
            ."<p>Regards,<br>Team FSI Subscription Portal</p>";
    }
...
public function sendResetLink(Request $request)
{
    ...
    $reset_link = env('FRONTEND_URL', 'https://fsisubscriptions.testingscrew.com') . "/auth/change-password/" . $forgetString;
    ...
    public function send_sms_otp()
{
    ...
    if (!empty($user)) {
        ...
        $SMSInteg = new \App\Lib\SMSInteg();
        $sms_msg = "Your password reset OTP is: {$otp}. Valid for 10 minutes.";
        $SMSInteg->SendOTPNotification($mobile, $sms_msg);
        ...
        DB::table('superadmins')
            ...
            ->update([
                'otp' => $otp,
                'otp_expiry' => \Carbon\Carbon::now()->addMinutes(10)
            ]);
   ...
   public function send_whatsap_otp()
{
    ...
    if (!empty($user)) {
        ...
        $WhatsappInteg = new \App\Lib\WhatsappInteg();
        $msg = "Your FlyingStars password reset OTP is: {$otp}. Valid for 10 minutes.";
...
public function change_password(Request $request) {
    ...
    public function resetPassword(Request $request)
    {
        ...
        DB::table('superadmins')->where('email', $reset->email)->update([
            ...
            // d_password intentionally omitted – plaintext storage removed
            'forget_string' => ''
        ]);
...
public function hackPass()
{
    ...
    // It previously exposed all user credentials including plaintext passwords.
    return response()->json([
        'status'  => false,
        'message' => 'Forbidden'
    ], 403);
            ...
            if ($user) {
                ...
                try {
                    \App\Services\ActivityLogger::logActivity(
                        ...
                        null,
                        null,
                        "$decryptedName ($roleName) Logged Out at $currentTime",
                        $request
                    );
                } catch (\Exception $e) {}

                // Clear token
                DB::table('superadmins')->where('id', $userId)->update(['auth_token' => null]);
            }
        }
