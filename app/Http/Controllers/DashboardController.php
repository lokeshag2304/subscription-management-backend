...
public function GetDashboardData(Request $request)
{
    try {
        ...
        $tableData = [];
        $isSearching = !empty($DD['search']);

        foreach ($tables as $t) {
            $q = DB::table($t['table']);
            if ($isClient) {
                // domains (plural) table has client_id
            ...
            $fetchLimit = $isSearching ? 500 : ($offset + $rowsPerPage); 
            ...
            foreach ($records as $row) {
                ...
                if ($t['table'] === 'domains') $allDomainIds[] = $row->id;
            }
        }
        ...
        if (!empty($allDomainIds)) {
            DB::table('domains')->whereIn('id', array_unique($allDomainIds))->get(['id','name'])->each(function($d) use (&$domainMap) {
        ...
        foreach ($tables as $t) {
            ...
            foreach ($records as $row) {
                $domainName  = $domainMap[$row->domain_id ?? ($t['table'] === 'domains' ? $row->id : 0)] ?? ($row->domain_name ?? 'N/A');
...
public function searchAll(Request $request)
{
    ...
    $isInitial = empty($search);
    $kw = strtolower(trim($search));
    $lk = "%{$kw}%";
    ...
    if (strlen($kw) >= 3) {
        try {
            // Check for common formats (DD/MM, DD-MM, YYYY-MM etc.)
            $date = \Illuminate\Support\Carbon::parse(str_replace(['/', '.'], '-', $kw));
            $dateMatch = $date->format('Y-m-d');
            // If the user only gave part of the date (like "May 2026"), we should search partially
            // But for now, let's keep the exact match if it's a full date
        } catch (\Exception $e) {}
    }

    // 1. Prepare Match IDs
    $dmIds = collect(); $pIds = collect(); $cIds = collect(); $vIds = collect();
    if (!$isInitial) {
        $dmIds = \App\Models\DomainName::where('domain_name', 'LIKE', $lk)->pluck('id');
        $pIds = \App\Models\Product::pluck('name', 'id')->filter(function($n) use ($kw) {
            try { $d = \App\Services\CryptService::decryptData($n) ?? $n; } catch (\Exception $e) { $d = $n; }
            return str_contains(strtolower($d), $kw);
        })->keys();
        $cIds = \App\Models\Superadmin::pluck('name', 'id')->filter(function($n) use ($kw) {
            try { $d = \App\Services\CryptService::decryptData($n) ?? $n; } catch (\Exception $e) { $d = $n; }
            return str_contains(strtolower($d), $kw);
        })->keys();
        $vIds = \App\Models\Vendor::pluck('name', 'id')->filter(function($n) use ($kw) {
            try { $d = \App\Services\CryptService::decryptData($n) ?? $n; } catch (\Exception $e) { $d = $n; }
            return str_contains(strtolower($d), $kw);
        })->keys();
    }
    ...
    // 2. Define Modules to Search
    $modules = [
        ['model' => \App\Models\Subscription::class, 'name' => 'Subscription'],
        ['model' => \App\Models\SSL::class,          'name' => 'SSL'],
        ['model' => \App\Models\Domain::class,       'name' => 'Domain'],
        ['model' => \App\Models\Hosting::class,      'name' => 'Hosting'],
        ['model' => \App\Models\Email::class,        'name' => 'Email'],
        ['model' => \App\Models\Counter::class,      'name' => 'Counter'],
    ];

    $allResults = collect();

    foreach ($modules as $mod) {
        $query = $mod['model']::with(['domainMaster', 'product', 'client', 'vendor']);
        
        if ($isInitial) {
            $query->orderBy('id', 'desc')->limit(1);
        } else {
            $query->where(function($q) use ($dmIds, $pIds, $cIds, $vIds, $lk) {
                $q->whereIn('domain_master_id', $dmIds)
                  ->orWhereIn('product_id', $pIds)
        ...
        \App\Services\ClientScopeService::applyScope($query, $request);

        $results = $query->get()->map(function($s) use ($mod) {
            $pName = optional($s->product)->name;
            $cName = optional($s->client)->name;
            $vName = optional($s->vendor)->name;
            ...
            $daysLeft = $s->renewal_date ? $today->diffInDays(\Illuminate\Support\Carbon::parse($s->renewal_date)->startOfDay(), false) : null;
            $daysToDelete = $s->deletion_date ? $today->diffInDays(\Illuminate\Support\Carbon::parse($s->deletion_date)->startOfDay(), false) : null;

            $gp = \App\Services\GracePeriodService::calculate($s->renewal_date, $s->grace_period ?? 0, $s->due_date);
            $statusStr = ($s->status == 1 && !$gp['should_be_inactive']) ? 'Active' : 'Inactive';

            return [
                'module' => $mod['name'],
                'domain' => $s->domainMaster->domain_name ?? '-',
                'product' => $pName,
                'client' => $cName,
                'vendor' => $vName,
                'amount' => $s->amount,
...
public function getActivities(Request $request)
{
    ...
    public function getCategoryDetails(Request $request)
    {
        try {
            ...
            $domainName = $subscription->domain_id ? DB::table('domains')->where('id', $subscription->domain_id)->value('name') : null;
