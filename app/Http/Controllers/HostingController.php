...
class HostingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $records = $query->get()->map(function ($item) {
                ...
                $item->updated_at_formatted = DateFormatterService::format($item->updated_at);
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            $importer = new \App\Imports\UniversalImport('hostings', $clientId);
            $result = \App\Services\ImportService::handleImport($request, $importer, 'Hosting');
            ...
            $latest = $inserted > 0
                ? Hosting::with(['product', 'client', 'vendor', 'domainInfo'])
                    ->orderBy('id', 'desc')
                    ->take($inserted)
                    ->get()
                    ->map(function ($item) {
                        ...
                        $dName = optional($item->domainInfo)->name; // Added domain name
                        ...
                        try { $dName = \App\Services\CryptService::decryptData($dName) ?? $dName; } catch (\Exception $e) {} // Decrypt domain name
                        $item->product_name = $pName;
                        $item->client_name  = $cName;
                        $item->vendor_name  = $vName;
                        $item->domain_name  = $dName; // Assign decrypted domain name
                        return $item;
                    })
                    ->reverse()
                    ->values()
