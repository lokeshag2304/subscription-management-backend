...
<body>
    <div class="container">
        ...
        <div class="vendor-info">
            <p><span class="bold">To: </span>{{$po_data->supplier_company_name}},</p>
            <p>{{ $po_data->supplier_address . ' ' . $po_data->pincode.' '.$po_data->city.'('.$po_data->state.')' }}</p>
            <p>Contact Person: {{$po_data->shipping_contact_person}}</p>
            <p>Phone: {{$po_data->supplier_mobile}}</p>
            <p>Email: {{$po_data->supplier_email}}</p>
            <p>GST No: {{$po_data->supplier_gst}}</p>
        </div>
        ...
        @php
    $gstType = strtolower($po_data->gst_type ?? '');
    $gstPercent = $po_data->gst_per ?? 0;
    $grandTotal2 = $subTotal;
@endphp
        <!-- TOTAL SECTION -->
        <div class="total-section">
    <!-- <p>Sub Total : {{$currency->currency_symbol}} {{ number_format($subTotal, 2) }}</p> -->
    ...
    <p><strong>Grand Total: {{$currency->currency_symbol}} {{ number_format($grandTotal, 2) }}</strong></p>
        ...
        <table class="combined-table">
            ...
            <tr>
                <td>
                    <!-- <strong>{{$po_data->company_name}}</strong><br> -->
                    {!! $po_data->company_billing_address !!}<br>
                    <!-- <strong>GST No:</strong> {{$po_data->company_gst_no}}<br>
                    <strong>Contact:</strong> {{$po_data->company_mobile}} -->
                </td>
                <td>
                    <!-- <strong>{{$po_data->company_name}}</strong><br> -->
            ...
            <tr style="border:none !important">
                <td colspan="2" style="border:none !important" class="signature-cell">
                <p><strong style="margin-top:12px">{{$po_data->company_name}}</strong><br><br>
                    @if(!empty($sign))
